...
**除 ETF 轮动回测外**，本系统中**量化回测**（普通回测、LLM 信号回测、AI Agent 量化中的回测）、**因子挖掘回测**、**选股池组合回测**均**未考虑滑点与手续费**。回测得到的收益、夏普、回撤等为理论值，实盘或高频调仓时交易成本会显著影响实际表现，请以回测结果仅作参考，实盘前需自行评估滑点与手续费影响。ETF 轮动模块的回测与模拟盘支持配置滑点与手续费，详见 [ETF轮动策略交易逻辑](etf_rotation_trading_logic.md)。
